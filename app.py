...
def highlight_code():
    ...
    out_file.write(render_template('index.jinja2', language=language, filename=filename, highlighted=highlighted, style=style))
    out_file.flush()
    os.fsync(out_file)

    subprocess.call(['wkhtmltoimage', '/tmp/highlighter/%s/index.html' % uuid, '/tmp/highlighter/%s/index.png' % uuid])

    resp = {"image_url": "http://localhost:5000/image/%s" % uuid, "page_url": "http://localhost:5000/page/%s" % uuid}
...
def page(uuid):
    return send_from_directory('/tmp/highlighter/%s' % uuid, 'index.html', as_attachment=not not request.args.get('download'))
